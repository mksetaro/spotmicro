...
class Robot:
    def __init__(self, useFixedBase=False, useStairs=True, resetFunc=None, urdf=""):
        ...
        for i in range(self.numRays):
            ...
            self.rayTo.append(
                [
                    ...
                    rayLen * math.cos(math.pi * 2 * float(i) / self.numRays),
                    h,
                ]
            )
            if self.debugLidar:
        ...
        # Camera Settings
        fov, aspect, nearplane, farplane = 90, 1.3, 0.0111, 100
    ...
    def createEnv(self):
        ...
        collisionShapeId = p.createCollisionShape(
            shapeType=p.GEOM_BOX, collisionFramePosition=shift, halfExtents=[1, 1, 0.5]
        )
        rangex = 5
        rangey = 5
    ...
    def changeDynamics(self, quadruped):
        ...
        for i in range(nJoints):
            p.changeDynamics(
                quadruped, i, localInertiaDiagonal=[0.000001, 0.000001, 0.000001]
            )
    ...
    def addInfoText(self, bodyPos, bodyEuler, linearVel, angularVel):
        ...
        newDebugInfo = [
            ...
            p.addUserDebugText(
                text, [x + 0.03, y, 0.6], textColorRGB=[1, 1, 1], textSize=1.0
            ),
            p.addUserDebugText(
                text2, [x + 0.03, y, 0.5], textColorRGB=[1, 1, 1], textSize=1.0
            ),
            p.addUserDebugText(
                text3, [x + 0.03, y, 0.4], textColorRGB=[1, 1, 1], textSize=1.0
            ),
        ]
        p.addUserDebugLine(
            [-0.3, 0, 0],
    ...
    def resetBody(self):
        ...
        p.resetBasePositionAndOrientation(
            self.quadruped, self.init_position, [0, 0, 0, 1]
        )
    ...
    def step(self):
        ...
        if nowLidarTime - self.lastLidarTime > 0.2:
            numThreads = 0
            ...
            for i in range(self.numRays):
                ...
                if hitFraction == 1.0:
                    ...
                    localHitTo = [
                        self.rayFrom[i][0]
                        + hitFraction * (self.rayTo[i][0] - self.rayFrom[i][0]),
                        self.rayFrom[i][1]
                        + hitFraction * (self.rayTo[i][1] - self.rayFrom[i][1]),
                        self.rayFrom[i][2]
                        + hitFraction * (self.rayTo[i][2] - self.rayFrom[i][2]),
                    ]
                    dis = math.sqrt(
                        localHitTo[0] ** 2 + localHitTo[1] ** 2 + localHitTo[2] ** 2
                    )
                    if self.debugLidar:
                        p.addUserDebugLine(
                            self.rayFrom[i],
